...
<div class="container">    
    <form class="d-flex ms-3 me-5 mt-2 mb-4 w-25" method="get" action="{{route('search')}}">
        @csrf
    ...
    <h5>Инструкции по эксплуатации:</h5>
    <table class="table table-striped table-info">
        @foreach($manuals as $manual)
            <tr>
                <td><div>{{$manual->description}}</div></td>
                <td><a href="{{route('manual_complaints',[$manual->manual_name])}}">{{$manual->manual_name}}</a></td>
                @if(Auth::check())
